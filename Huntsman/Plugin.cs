...
{
    private const string CommandName = "/huntsman";
        ...
        IPluginLog log)
    {
        ...
        var monsterNavigator = new MonsterNavigator(services, config, lifestream, vnavmesh, rotationDriver, commandDispatcher, monsterRoutePlanner, mountService);
        var huntController = new HuntController(services, dropHuntList, combatJobs, monsterNavigator);
        window = new MainWindow(config, lifestream, vnavmesh, rotationDriver, monsterNavigator, dropLocations, planner, dropHuntList, huntController, combatJobs);

        commands.AddHandler(CommandName, new CommandInfo(OnCommand)
        {
            HelpMessage = "Open Huntsman.",
        });
    ...
    private void OnCommand(string command, string args)
    {
        ...
        switch (arg)
        {
            ...
            case "stop":
                window.StopHunt();
                services.Chat.Print("Huntsman: stopped navigation and hunt automation.");
                break;
            default:
                services.Chat.Print("Usage: /huntsman [open|status|stop]");
                break;
        }
    }
}
